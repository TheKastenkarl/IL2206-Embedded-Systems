...
procedure overloaddetection is
   ...
   -- Helper task used to reset the watchdog timer
   task helper_task is
      pragma Priority(1);
   end;
   
   -- Watchdog timer task must have the highest priority so that it is cannot be blocked by other tasks
   task watchdog is
      pragma Priority(25);
      entry resetTimer;
   end;
   ...
   begin
    ...
    -- reset the watchdog timer when no other task is running (therefore this task must have the lowest priority)
    ...
    begin
        ...
        loop
            ...
            watchdog.resetTimer;
            --delay until Next; -- use delay to save computations
        end loop;
   end helper_task;
    
    -- Warning is printed if the delay is not reset for 1.2s (= hyperperiod)
    -- The reset is triggered by the helper task
    task body watchdog is 
        --Next : Time;
    begin
        delay Warm_Up_Time * 0.001; -- consider phase of tasks (convert ms to s)
        loop
            select
                accept resetTimer do -- reset the watchdog timer
                    --delay 1.0;
                    --Put_Line("resetTimer");
                    null;
                end resetTimer;
            or
                delay 1.2; -- hyperperiod
                --Next := Clock + Milliseconds(1200); -- hyperperiod
                Put_Line("Warning: Overload detected!");
                --delay until Next;
            end select;
        end loop;
    end watchdog;
